...
export const getEventsByCategory = (category: string) => allEvents.filter(event => event.category === category)

export const getFeaturedEvents = () => allEvents.filter(event => event.featured).slice(0, 4)
